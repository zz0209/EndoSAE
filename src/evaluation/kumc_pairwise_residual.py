"""Within-frame pairwise ranking of frozen-base residual readouts."""
import argparse
import json
import os
import time
...
def fit(run,config,torch):
    ...
    for method in config['methods']:
        ...
        for step in range(config['steps']):
            ...
            loss=pairwise_support_loss(logits,labels,ids)
            ...
            if (step+1)%50==0:history.append({'step':step+1,'pairwise_logistic':float(loss.detach())})
